...
$categories = [];
$categoryResult = $conn->query("SELECT * FROM danhmucsanpham WHERE id_DanhMuc IN (1, 2, 3) ORDER BY FIELD(id_DanhMuc, 1, 2, 3)");
if ($categoryResult) {
...
$categoryMeta = [
    ...
    3 => [
        ...
        'description' => 'Sản phẩm thời trang dành cho Trẻ em',
    ],
];
                ...
                <div class="row g-3">
                    <?php foreach ($categories as $category): ?>
                        ...
                        $meta = $categoryMeta[$categoryId] ?? [
                            'icon' => 'fa-tag',
                            'description' => 'Xem sản phẩm trong danh mục này',
                        ];
                        ?>
                        <div class="col-12 col-md-4">
                            <a href="sanpham.php?danhmuc=<?= $categoryId ?>" class="text-decoration-none text-dark" aria-label="Xem sản phẩm danh mục <?= htmlspecialchars($category['Ten_DanhMuc']) ?>">
                                <article class="card category-card text-center h-100">
                                    <div class="card-body d-flex flex-column align-items-center justify-content-center">
                                        <i class="fas <?= $meta['icon'] ?> fa-2x mb-3"></i>
                                        <h5 class="card-title mb-2"><?= htmlspecialchars($category['Ten_DanhMuc']) ?></h5>
                                        <p class="category-description mb-0"><?= htmlspecialchars($meta['description']) ?></p>
                                    </div>
                                </article>
                            </a>
                        </div>
